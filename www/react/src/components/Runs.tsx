  ...
  render() {
    ...
    return <div>
      <ButtonToolbar style={{paddingBottom: 8}}>
        <DropdownButton style={{minWidth: 120}} title={"Nick: " + this.state.selectedNick} id="bg-nested-dropdown" onSelect={this.onNickSelect.bind(this)}>
        ...
        <DropdownButton style={{minWidth: 120}} title={"Task: " + this.state.selectedTask} id="bg-nested-dropdown" onSelect={this.onTaskSelect.bind(this)}>
      ...
      <div style={{height: "512px", overflow: "scroll"}}>
        <ListGroup>
          {runs.filter((run) => {
            ...
            return <ListGroupItem key={run.run_id}>{run.run_id} - {run.info.nick}</ListGroupItem>;
          })}
        </ListGroup>
      </div>
    </div>
